...
def create_tbl(ID, nextclade_csv):
   ...
   if variants:
      ...
      for variant in unique_variants:
         ...
         df[col_name] = df["aaSubstitutions"].apply(lambda x: variant in x if isinstance(x,str) else False)
         df.loc[df[col_name],col_name] = variant #replace all values that are True with the respective variant name
   ...
   if aaDeletions:
      ...
      for aaDeletion in unique_aaDeletions:
         ...
         df[col_name] = df["aaDeletions"].apply(lambda x: aaDeletion in x if isinstance(x,str) else False)
         df.loc[df[col_name],col_name] = aaDeletion #replace all values that are True with the respective deletion name
   ...
   if aaInsertions:
      ...
      for aaInsertion in unique_aaInsertions:
         ...
         df[col_name] = df["aaInsertions"].apply(lambda x: aaInsertion in x if isinstance(x,str) else False)
         df.loc[df[col_name],col_name] = variant #replace all values that are True with the respective variant name
